...
<body class="flex min-h-screen">
    ...
    <main class="flex-1 ml-64 p-8 h-screen flex flex-col overflow-hidden">
        ...
        <div class="flex justify-between items-end mb-8 shrink-0">
            <div>
                <h2 class="text-3xl font-[900] text-slate-900 tracking-tighter italic uppercase">Arena <span class="text-emerald-500">Timeline</span></h2>
                <p class="text-slate-400 font-bold italic text-sm mt-1">Vos prochains rendez-vous et votre historique de performance.</p>
            </div>
            ...
            <div class="bg-slate-900 px-6 py-4 rounded-[2rem] shadow-xl border border-slate-800 flex items-center gap-4">
                <div class="text-center border-r border-slate-700 pr-4">
                    <p class="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-1 italic">Total XP</p>
                    <p class="text-xl font-[900] text-white italic leading-none">{{ Auth::user()->xp_points }}</p>
                </div>
                <div>
                    <p class="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 italic">Niveau Actuel</p>
                    <p class="text-lg font-[900] text-white italic leading-none">{{ Auth::user()->level->level_name ?? 'ROOKIE' }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="w-1/3 flex flex-col min-h-0">
                <div class="flex items-center justify-between mb-4 px-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Prochains Matchs</h3>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
                    ...
                    <div class="group bg-slate-50 rounded-[2.5rem] p-6 border border-transparent hover:border-emerald-200 hover:bg-white transition-all duration-500">
                        <div class="flex items-center gap-4 mb-4">
                            <div class="w-12 h-12 bg-white rounded-2xl flex flex-col items-center justify-center shadow-sm border border-slate-100">
                                ...
                                <span class="text-lg font-[900] text-slate-900 italic leading-none">{{ \Carbon\Carbon::parse($match->start_time)->format('d') }}</span>
                            </div>
                            <div>
                                <p class="text-xs font-black text-slate-900 uppercase italic tracking-tight">{{ $match->court->name }}</p>
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-slate-100">
                            <div class="flex -space-x-2">
                                ...
                                @endif
                                <div class="w-7 h-7 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[8px] text-slate-400 italic font-black" title="En attente d'adversaires">?</div>
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="h-full flex flex-col items-center justify-center text-center px-6">
                        ...
                        <p class="text-[10px] font-black uppercase italic tracking-widest text-slate-300">Aucun match de prévu</p>
                    </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div class="flex items-center justify-between mb-4 px-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Historique des Matchs</h3>
                    </div>
...
<div id="match-{{ $match->id }}"
    ...
    {{-- Infos --}}
    <div class="flex-1">
        <p class="text-sm font-black text-slate-900 uppercase italic tracking-tight leading-none mb-1 group-hover:text-emerald-600 transition-colors">{{ $match->court->name }}</p>
        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic">{{ $match->total_price }} PC — {{ \Carbon\Carbon::parse($match->start_time)->format('H:i') }}</p>
    </div>
    ...
    <div class="flex flex-col gap-1.5">
        <button onclick="setResult({{ $match->id }}, 'win')" id="win-{{ $match->id }}"
            class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase italic transition-all
        ...
        <button onclick="setResult({{ $match->id }}, 'loss')" id="loss-{{ $match->id }}"
            ...
            <i class="fas fa-times text-[8px]"></i> Défaite
                            ...
                            <div class="h-64 flex flex-col items-center justify-center opacity-40">
                                ...
                                <p class="text-[10px] font-black uppercase italic tracking-widest text-center">Aucun match trouvé pour<br>cette période</p>
                            </div>
                            @endforelse
                        </div>
                    </div>
        ...
        <div class="mt-8 py-6 px-10 bg-white/70 backdrop-blur-md rounded-[3rem] border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex justify-between items-center shrink-0">
            <div class="flex gap-12">
                ...
                <div class="flex items-center gap-4 group">
                    ...
                    <div>
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-0.5">Ratio Victoire</p>
                        <p id="win-rate-display" class="text-2xl font-[900] text-slate-900 italic uppercase tracking-tighter leading-none">{{ $winRate }}%</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 group">
                    ...
                    <div>
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-0.5">Temps de jeu</p>
                        <p class="text-2xl font-[900] text-slate-900 italic uppercase tracking-tighter leading-none">{{ $totalPlayTime }}H</p>
                    </div>
                </div>
            </div>
            ...
            <div class="text-right">
                <div class="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 rounded-full shadow-lg shadow-slate-200">
                    ...
                    <span class="text-[9px] font-black text-white uppercase italic tracking-widest">Saison 2026</span>
                </div>
            </div>
        </div>
...
function setResult(matchId, result) {
    ...
    .then(data => {
        ...
        winBtn.className = `flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase italic transition-all ${data.result === 'win' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200' : 'bg-slate-100 text-slate-400 hover:bg-emerald-100 hover:text-emerald-600'}`;

        // 4. Mise à jour du bouton Défaite
        const lossBtn = document.getElementById(`loss-${matchId}`);
        lossBtn.className = `flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase italic transition-all ${data.result === 'loss' ? 'bg-red-500 text-white shadow-lg shadow-red-200' : 'bg-slate-100 text-slate-400 hover:bg-red-100 hover:text-red-500'}`;
    });
}
</script>
